...
# Fields
  ...
  - `coeff`: fitted coefficients, laid out as `[intercept; slopes]`. For
    vector-valued responses this is a matrix with one column per output.
...
# Arguments
  ...
  - `y`: training responses, with one response per element of `x`. Responses may
    be scalars or equal-length vectors; vector-valued responses are fitted one
    output per column and returned in the same container.
...
_linear_coeff(x, y) = _linear_design_matrix(x) \ _construct_y_matrix(y, first(y))

function LinearSurrogate(x, y, lb, ub)
    return LinearSurrogate(x, y, _linear_coeff(x, y), lb, ub)
end

# `coeff[k, :]` is a one-element vector when the responses are scalars and a
# row of length m when they are vectors, so one expression serves both; the
# result is unwrapped back to a scalar by `_match_container`.
function (lin::LinearSurrogate)(val::Number)
    _check_dimension(lin, val)
    y = lin.coeff[1, :] + lin.coeff[2, :] * val
    return _match_container(y, first(lin.y))
end
