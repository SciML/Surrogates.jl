...
function I_tier_ranking_ND(P,surrSOPn::AbstractSurrogate)
    #obj1 = objective_function
    #obj2 = obj2_ND
    Fronts = Dict{Int,Array{eltype(surrSOPn.x[1]),1}}()
    i = 1
    while true
        F = []
        j = 1
...
function II_tier_ranking_ND(D::Dict,srg::AbstractSurrogate)
    for i = 1:length(D)
        pos = []
        yn = []
        for j = 1:length(D[i])
...
function surrogate_optimize(obj::Function,sopn::SOP,lb,ub,surrSOPn::AbstractSurrogate,sample_type::SamplingAlgorithm;maxiters=100,num_new_samples=100)
    d = length(lb)
    N_fail = 3
    N_tenure = 5
    tau = 10^-5
    num_P = sopn.p
    centers_global = surrSOPn.x
    r_centers_global = 0.2*norm(ub-lb)*ones(length(surrSOPn.x))
    N_failures_global = zeros(length(surrSOPn.x))
    tabu = []
    N_tenures_tabu = []
    for k = 1:maxiters
        N_tenures_tabu .+= 1
        ...
        #Rank points in S with:
        #1) Non dominated sorting
        Fronts_I = I_tier_ranking_1D(centers_global,surrSOPn)
        #2) Second tier ranking
        Fronts = II_tier_ranking_1D(Fronts_I,surrSOPn)
        ranked_list = []
        for i = 1:length(Fronts)
            for j = 1:length(Fronts[i])
                push!(ranked_list,Fronts[i][j])
        ...
        ranked_list = eltype(surrSOPn.x[1]).(ranked_list)
        ...
        N_failures = zeros(num_P)
        #2.3 Candidate search
        new_points = zeros(eltype(surrSOPn.x[1]),num_P,2)
        for i = 1:num_P
            N_candidates = zeros(eltype(surrSOPn.x[1]),num_new_samples)
            #Using phi(n) just like DYCORS, merit function = surrogate
            #Like in DYCORS, I_perturb = 1 always
            evaluations = zeros(eltype(surrSOP.y[1]),num_new_samples)
            for j = 1:num_new_samples
            ...
            """x_best = N_candidates[argmin(evaluations)]"""
	       X_best = collect(surrSOP.x[N_candidates[argmin(evaluations)]])
        ...
        for i=1:num_P
            ...
            f_1 = new_points[i,1]
            f_2 = obj2_ND(f_1,surrSOP.x)

            l = length(Fronts[1])
            Pareto_set = zeros(eltype(surrSOPn.x[1]),l,2)

            for j = 1:l
                val = obj2_ND(Fronts[1][j],surrSOPn.x)
                Pareto_set[j,1] = Fronts[1][j]
                Pareto_set[j,2] = val
            end
            if (Hypervolume_Pareto_improving(f_1,f_2,Pareto_set)<tau)
                ...
                #P_i is success
                #Adaptive_learning
                add_point!(surrSOPn,new_points[i,1],new_points[i,2])
                push!(r_centers_global,r_centers[i])
                push!(N_failures_global,N_failures[i])
            end
        end
