...
function _calc_gek_coeffs(x,y,p::Number,theta::Number)
    ...
    @inbounds for i = n+1:nd1
        for j = n+1:nd1
            R[i,j] = -4*theta*(x[i-n] - x[j-n])^2*exp(-theta*abs(x[i-n]-x[j-n])^p)
        end
    end
...
function _calc_gek_coeffs(x,y,p,theta)
    ...
    @inbounds for i = 1:n
        for j = 1:n
            R[i,j] = prod([exp(-theta[l]*(x[i][l] - x[j][l])) for l = 1:d])
        end
    end
    ...
    #bottom right
    ir = 1
    @inbounds for i = n+1:d:nd
        for j = n+1:d:nd
            jr = 1
            for l = 1:d
                for k  = 1:d
                    R[i+l-1,j+k-1] = -4*theta[l]*theta[k]*(x[ir][l]-x[jr][l])*(x[ir][k]-x[jr][k])*R[ir,jr]
                end
            end
            jr = jr + 1
        end
